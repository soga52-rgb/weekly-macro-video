...
在建立影片主線與分鏡前，必須先完成以下總經傳導鏈判斷：

1. 先判斷本週通膨預期是否形成明確方向。
   請綜合檢查物價硬數據、能源價格、供給風險、需求景氣、勞動市場、薪資、Fed 訊號與市場價格反應。
   結論請歸類為 strong / mixed / weak / unclear，並說明支持訊號與抵銷訊號。

2. 若通膨預期為 strong，下一步才判斷利率是否主要受通膨預期推動。
   同時仍需檢查 Fed policy_guidance、term_premium、bond_supply_demand 是否有獨立影響。

3. 若通膨預期為 mixed / unclear，但長端利率仍上升，請把利率上升的主因優先檢查為：
   policy_guidance、term_premium、bond_supply_demand、long-end repricing 或 mixed。
   此時不可把利率上升直接歸因於通膨預期升溫。

4. 接著檢查利率與通膨預期是否同步。
   若不同步，請把這個不同步本身視為本週重要總經訊號。

5. 最後才分析美元、亞洲貨幣、黃金與其他資產如何反映前面形成的通膨 / 利率判斷。
"""
...
1. scene_control_list
   ...
   scene_control_list 不需要硬性固定每一幕主題，但影片敘事應遵守總經傳導鏈的診斷順序：
   1. 開場先提出本週核心市場問題。
   2. 接著先處理通膨預期是否形成明確方向，或說明為何通膨訊號呈現 mixed / unclear。
   3. 再判斷利率預期的主要驅動來源，是通膨預期、Fed policy_guidance、term_premium、bond_supply_demand，或其他因素。
   4. 然後檢查利率、美元、亞洲貨幣、黃金與能源之間是否同步或背離。
   5. 最後整理下週觀察節點。

   scene_control_list 的敘事順序應以總經診斷順序為主，而不是以新聞衝擊強度排序。
   除 overview 開場外，應先呈現通膨預期是否形成明確方向，再呈現利率驅動來源。

   若本週通膨預期訊號不明確，影片中應有一幕或一段明確說明：
   - 支持通膨預期升溫的訊號
   - 抵銷通膨預期的訊號
   - 為何本週無法形成單一方向的通膨敘事
...
Tom / Miranda 的對話素材應遵守總經傳導順序：
先確認通膨預期是否明確形成，再討論利率是否由通膨預期驅動，或由 Fed 訊號、期限溢價、長債供需等因素獨立推動。
若通膨預期 mixed / unclear，Miranda 應明確說明利率上升不是單純通膨交易，而是其他政策或債券市場因素在主導。
"""
...
三層對齊時，請優先保留總經診斷順序：
通膨預期形成與否 → 利率驅動來源 → 跨資產同步 / 背離 → 美元、亞幣、黃金反應 → 下週觀察。

除 overview 開場外，scene_control_list 應先呈現通膨預期是否形成明確方向，再呈現利率驅動來源。
"""
