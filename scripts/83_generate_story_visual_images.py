...
如果生成 overview 圖，overview 應根據 story_thesis、storyline.main_event_threads 與 next_week_watch，呈現整週故事總覽。
全部圖片要像同一個系列，風格、筆觸、構圖語言要一致。

核心風格要求：
1. 使用白板手繪風格、手繪圖解、極簡線條插畫。
2. 背景乾淨，以白底或極淡底色為主。
3. 色彩選用以高對比點綴色為主，少量使用、清楚醒目，避免低飽和或接近背景色的顏色。
4. 每張圖只保留一個視覺焦點，使用少量繁體中文短標籤與關鍵數字，避免塞滿資訊。
5. 使用簡單符號、icon、箭頭、線條與簡單圖表，畫面要直覺。
6. 不要做成傳統 PPT、密集表格、複雜簡報頁、3D 科技感或策展感。
7. 若畫外匯或匯率，請清楚標示是匯率報價還是本幣方向。例如：USD/TWD 下降代表台幣升值；USD/KRW 下降代表韓元升值；USD/JPY 上升代表日圓偏弱。
""".strip()
