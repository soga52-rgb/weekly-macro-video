...
- Generate weekly_forest_summary.json from:
  ...
  3) weekly_news_context.md / weekly_news_context.json if available

Design:
- Use weekly_v35_diagnosis.json as the primary structured diagnosis.
- Use market data and news context to validate, explain, correct, or challenge the V35 diagnosis.
...
weekly_v35_diagnosis.json（rule-based V35 結構化診斷，優先使用）：
...
def main() -> None:
    ...
    print(f"[INFO] Rule-based V35 diagnosis included: {bool(authoritative_v35_diagnosis)}")
    ...
    # Keep the external rule-based compact diagnosis authoritative after Gemini
    # and normalization. This prevents empty model output from blocking the
    # upstream V35 diagnosis and keeps all downstream steps on the same base.
    if authoritative_v35_diagnosis:
        forest_summary["weekly_v35_diagnosis"] = authoritative_v35_diagnosis

    forest_summary.setdefault("meta", {})
    forest_summary["meta"]["source"] = "weekly_market_series.json + weekly_news_context.md/json + weekly_v35_diagnosis.json"
