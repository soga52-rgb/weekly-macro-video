...
- It uses principle-based classification so different weeks can show different natural news weights.
...
背景分類與判斷原則：
- 這不是為了固定保留某一個特定標的或新聞，而是為了判斷近 2～4 週內哪些背景資訊仍會影響本週市場預期。
- 請依新聞自然比重分類，不要平均分配，也不要為了湊類別而放入低相關新聞。
- 若某類背景資訊在本週市場中明顯較重要，可以讓該類內容較多；若某類資料不足，請標示資料不足或待觀察。
- 不要因為本週單一事件，就直接覆蓋前期仍具市場影響力的背景資料。
- 每則新聞請依「最主要影響路徑」分類；若同時影響多個面向，可在 why_it_matters 說明，但不要重複放入多個類別。
- Fed 鷹派、會議紀錄、升息選項，通常屬於 policy_guidance 或 rate_expectation；只有在新聞明確指向通膨壓力時，才歸為 support_inflation。
- 長天期殖利率上升、全球長債拋售、長債突破關鍵水準，通常屬於 term_premium 或 rate_expectation；不要自動歸類為通膨。
- 長債拍賣需求疲弱、財政赤字、債務供給壓力，通常屬於 bond_supply_pressure。
- 油價變化通常先代表能源端或地緣風險修正，不可單獨推論整體通膨預期已明確轉強或轉弱。
- 若背景資訊多空交錯，請在 background_assessment_for_91 中明確提醒 91：應判斷為 mixed / unclear / 待確認，而不是硬下單邊結論。
...
判斷重點：
- 請根據近 2～4 週資料，判斷哪些背景資訊仍會影響本週市場預期。
- 不要為特定標的、固定新聞或固定類別硬設保留名額；請讓新聞自然比重反映市場關注度。
- 不要平均分配，也不要為了湊數放入低相關新聞；每類數量可多可少。
- 若某項背景資訊雖非本週新聞，但仍會影響本週通膨、利率、美元、避險或匯率判斷，應納入背景評估。
- Fed 政策、長債殖利率、期限溢價、長債供需，應依主要影響路徑分別歸入 fed_policy_background、global_bond_background、fiscal_term_premium_background，不要硬塞進 inflation_background。
...
- 若背景資料多空交錯，請明確提醒 91：應判斷為 mixed / unclear / 待確認，而不是只因本週單一事件就判斷 strong 或 weak。
