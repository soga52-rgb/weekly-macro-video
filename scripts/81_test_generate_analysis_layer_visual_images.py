...
- 每頁僅保留必要資訊，避免過多卡片、過多 icon、過多箭頭；導入頁優先使用 icon grid 與細條數據列。
...
上半部的新聞脈絡只放 icon 與極短標籤，不要放完整敘述句。
可以保留「當週新聞」「前期新聞」兩個小標，但字級要小、不要搶畫面。
文字只使用「當週新聞」「前期新聞」，不要寫「當週新增新聞」或「前期背景新聞」。

新聞區塊的卡片要像 icon grid 或輕量標籤列：
- 以 icon 為主，文字為輔。
- 每個新聞標籤盡量控制在 2～8 字。
- 不要把語音稿內容畫進圖裡。
- 不要寫完整因果句。
- 不要在卡片右下角顯示括號標籤，如「（強化）」「（抵銷）」「（分歧）」等。
- 卡片必須融入各自白底區塊，使用白底細框、極淡同色系底卡、弱陰影或直接融入區塊。

每個主題區塊最多放 3～5 個新聞 icon / 短標籤；若素材過多，請優先濃縮。
新聞短標籤範例：
- 通膨：美伊和平、CPI / PPI、油價反覆
- 利率：30Y 5.1%、Fed 紀錄、房市疲軟、關稅、能源
- 美元 / 亞幣 / 黃金：美元指數強勢、亞幣弱勢、黃金震盪

下半部的市場驗證請做成「細條式數據列」，不要再做大卡片區。
每個主題區塊只放 1 條細條數據列，最多 2～3 個短數據點。
細條數據列範例：
- 通膨：WTI 105.4 → 97.7｜Brent 109.2 → 104.5
- 利率：US10Y 4.57–4.66｜30Y > 5.1%
- 美元 / 亞幣 / 黃金：DXY 98.9–99.3｜USDJPY 159｜Gold 4480–4577
